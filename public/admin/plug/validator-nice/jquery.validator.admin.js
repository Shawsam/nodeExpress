...
(function(factory) {
    ...
    $.validator.config({
        ...
        validation: function(form, errors){
        	...
        	var  obj =$(errors.element).closest(".form-group");
        	...
        	//alert()
        	var obj = $(elem).closest(".form-group");
        	 if(obj.find("div").length==2){
        	   obj=$(elem).closest('.form-group>div');
        	 }
        	 var $formitem = obj,
	         $msgbox = $formitem.find('small.help-block');
	        if (!$msgbox.length) {
	            $msgbox = $('<small   class="help-block"></small>').appendTo($formitem);
	        }
	        return $msgbox;
        }
    });
